...
describe("page navigation", () => {
  it("sends a chatbot message and displays the Kafka-backed response", async () => {
    ...
    api.mockImplementation((path) => {
      ...
      if (path === "/api/chatbot/messages")
        return Promise.resolve({
          ...
          message: "Siparişinizi Siparişlerim ekranından takip edebilirsiniz.",
        });
      return Promise.resolve(null);
    });
    ...
    await user.type(screen.getByLabelText("Mesajınız"), "Kargom nerede?");
    await user.click(screen.getByRole("button", { name: "Gönder" }));

    expect(api).toHaveBeenCalledWith("/api/chatbot/messages", {
      method: "POST",
      body: JSON.stringify({ message: "Kargom nerede?" }),
    });
    expect(
      await screen.findByText(
        "Siparişinizi Siparişlerim ekranından takip edebilirsiniz.",
      ),
    ).toBeInTheDocument();
  });
